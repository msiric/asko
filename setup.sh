...
print_summary() {
    ...
    echo ""
    echo -e "  ${BOLD}Quick access (choose one):${NC}"
    echo "    A) Direct: http://localhost:80"
    echo "    B) Local DNS — add to /etc/hosts:"
    local machine_ip
    machine_ip=$(hostname -I 2>/dev/null | awk '{print $1}' || echo "127.0.0.1")
    echo "       ${machine_ip} chat.${domain_base} n8n.${domain_base} ai.${domain_base}"
    echo "    C) Tailscale (recommended for remote access): see docs/TAILSCALE.md"
    echo ""
    echo -e "  ${BOLD}Optional setup:${NC}"
    echo "    1. Telegram bot: @BotFather → /newbot → add token in n8n credentials"
    echo "    2. Cloudflare Tunnel for webhooks: cloudflared tunnel --url http://localhost:5678"
    echo ""
